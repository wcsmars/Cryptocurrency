...
class CryptoVolatilityAnalyzer:
        ...
        for file in os.listdir(self.path):
            ...
            if file.lower() == 'current crypto leaderboard.csv':
                ...
                try:
                    ...
                    # Historical volatility of the last 30 data points (annualized) as instructed
                    hist30_returns = df['Returns'].tail(30)
                    hist30_volatility = hist30_returns.std() * np.sqrt(365)
                    self.hist30_vol[df_name] = hist30_volatility

                    # Normality Tests
                    returns_data = df['Returns'].tail(100)  # Last 100 data points
    ...
    def plot_rolling_statistics(self, df_name, window=30):
        """
    ...
    def residual_analysis(self, df_name, model_type):
        ...
        standardized_residuals = res.resid
    ...
    def fit_garch(self, p=1, q=1, dist='studentst', rescale=True):
        ...
        for df_name, df in self.dataframes.items():
            hist30_returns = df['Returns'].tail(30)
            try:
                model = arch_model(hist30_returns, vol='Garch', p=p, q=q, dist=dist, rescale=rescale)
                res = model.fit(update_freq=5, disp='off')

                if res.convergence_flag == 0:
                    last_vol = np.sqrt(res.conditional_volatility.iloc[-1] ** 2) * np.sqrt(365)
    ...
    def fit_tgarch(self, p=1, q=1, o=1, dist='studentst', rescale=True):
        ...
        for df_name, df in self.dataframes.items():
            hist30_returns = df['Returns'].tail(30)
            try:
                model = arch_model(hist30_returns, vol='Garch', p=p, q=q, o=o, dist=dist, rescale=rescale)
                res = model.fit(update_freq=5, disp='off')
    ...
    def fit_egarch(self, p=1, q=1, dist='studentst', rescale=True, maxiter=10000, tol=1e-10):
        ...
        for df_name, df in self.dataframes.items():
            hist30_returns = df['Returns'].tail(30)
            try:
                model = arch_model(hist30_returns, vol='EGarch', p=p, q=q, dist=dist, rescale=rescale)
    ...
    def rank_volatility(self, days=30):
            """
    ...
    def forecast_volatility(self, df_name, model_type, steps, method, simulations):
        ...
        hist30_returns = df['Returns'].tail(30)
        train = hist30_returns.iloc[:-steps]
        test = hist30_returns.iloc[-steps:]
...
def main():
    ...
    # analyzer.plot_relative_volume(days=30, top_n=50, chart_type='bar')
    ...
    ''' As I am using only 30 data points to forecast volatility, MSE is high, which is expected.'''
    ''' Using historical volatility might be more appropriate for predicting volatility with 30 days window. '''
    ...
    ''' As I can see from the results, eGARCH dominates the other two GARCH models for predicting cryptocurrency volatility. '''
...
EGARCH often performs better in volatility forecasting for cryptocurrencies, likely due to its ability to handle asymmetry and skewed distributions.

Feature engineering could be applied to improve predictability.

Longer and more frequent data points would yield better result for volatility.

Focusing on one specific cryptocurrency would produce better result with more suitable distribution and assumptions.

More predictive models could be applied beyond GARCH models, such as LSTM, or build upon the existing GARCH models.

Optimizing hyperparameters for GARCH would also likely to produce better AIC and BIC.

Graphs and plots can be produced by uncommenting the lines in the main function.

Residual analysis and other statistical tests are also available with the functions above.

The tables below are some of the summary statistics.
...
--------------------------------------------------------------------------------
bitcoin - wrapped bitcoin                              0.9992
binance usd - trueusd                                  0.9374
binance usd - pax dollar                               0.8618
bitcoin bep2 - wrapped bitcoin                         0.8317
ethereum - wrapped bitcoin                             0.8136
litecoin - wrapped bitcoin                             0.7972
dash - zcash                                           0.7885
cardano - stellar                                      0.7815
cardano - ethereum                                     0.7759
ftx token - wrapped bitcoin                            0.7758

Maximum Drawdown for Each Cryptocurrency:
-----------------------------------------
   Cryptocurrency Max Drawdown
0          tether      -10.32%
1         trueusd      -10.54%
2             dai      -11.48%
3         fei usd      -18.25%
...
Sharpe Ratio for Each Cryptocurrency: (Assuming 4% annual risk-free rate)
--------------------------------------
        Cryptocurrency  Sharpe Ratio
0               solana      2.271749
1        axie infinity      1.364836
2   trust wallet token      1.303594
3            ftx token      0.969525
4               helium      0.925917
..                 ...           ...
93     curve dao token     -0.648627
94               stepn     -0.860529
95                mina     -0.886108
96                usdd     -1.029936
97   internet computer     -2.259250
...
monero                   0.117516
lido dao                 0.116032
convex finance           0.107727

Ranked Latest Close Prices:
----------------------------
wrapped bitcoin          2.131046e+04
bitcoin                  2.130900e+04
bitcoin bep2             2.130399e+04
pax gold                 1.731405e+03
ethereum                 1.623240e+03
maker                    8.449557e+02
bnb                      2.972076e+02
gnosis                   1.657234e+02
monero                   1.545630e+02
bitcoin cash             1.233000e+02
quant                    1.061654e+02
solana                   9.381700e+01
aave                     8.871758e+01
zcash                    6.504274e+01
litecoin                 5.688000e+01
bitcoin sv               5.449917e+01
elrond                   5.432909e+01
kusama                   5.067561e+01
dash                     4.734606e+01
ethereum classic         3.381790e+01

Top 50 Cryptocurrencies by Dollar Volume over the last 30 days:
------------------------------------------------------------
Rank  Cryptocurrency              Dollar Volume
------------------------------------------------------------
1     bnb                  $12,183,403,985,987.92
2     bitcoin bep2         $8,784,936,599,810.55
3     maker                $7,299,645,123,773.03
4     pax gold             $ 960,273,530,228.36
5     aave                 $ 712,136,993,270.64
6     zcash                $ 187,657,820,655.10
7     binance usd          $ 175,133,793,189.36
8     quant                $ 170,029,583,054.17
9     dash                 $ 142,662,384,114.47
10    bitcoin              $ 139,862,328,981.60
11    filecoin             $ 129,793,380,836.09
12    bitcoin sv           $ 108,481,354,104.55
13    elrond               $  96,299,190,959.99
14    axie infinity        $  94,544,160,381.20
15    kusama               $  72,259,014,929.59
16    ethereum             $  55,378,789,805.50
17    bitcoin gold         $  50,619,721,064.74
18    waves                $  46,377,164,840.34
19    gnosis               $  33,146,952,441.86
20    internet computer    $  25,725,254,682.85
21    neo                  $  19,928,225,290.20
22    arweave              $  16,443,714,203.15
23    eos                  $  16,109,400,825.59
24    decred               $  15,324,530,471.08
25    trueusd              $  14,305,399,757.55
26    flow                 $  13,965,729,732.91
27    the sandbox          $  13,180,659,813.27
28    synthetix            $  12,141,279,249.43
29    qtum                 $  11,462,279,283.93
30    thorchain            $  10,597,467,526.15
31    lido dao             $  10,023,785,500.45
32    okb                  $   8,408,440,459.43
33    usd coin             $   7,789,634,845.40
34    pancakeswap          $   7,580,427,828.69
35    ethereum classic     $   7,301,831,496.79
36    curve dao token      $   7,289,508,141.17
37    decentraland         $   6,856,295,295.29
38    xrp                  $   4,338,824,656.92
39    cardano              $   3,619,607,409.04
40    dogecoin             $   3,547,877,106.79
41    theta network        $   3,346,118,852.72
42    tether               $   3,267,319,584.27
43    polygon              $   2,920,393,677.94
44    fantom               $   2,874,165,359.45
45    convex finance       $   2,747,386,725.37
46    tezos                $   2,585,534,489.76
47    avalanche            $   2,353,791,711.19
48    helium               $   2,350,996,033.58
49    kava                 $   2,181,472,318.92
50    huobi token          $   2,158,524,402.62
------------------------------------------------------------
Total Volume: $31,705,367,749,160.97
...
apecoin                  141.802609
stepn                    106.434822
binance usd              102.320363
neutrino usd              99.325758
bitcoin                   88.253823
trueusd                   83.825431
compound                  68.505515
celsius                   50.977805
pax dollar                44.512793
celo                      41.796104
the graph                 41.433836
monero                    40.266726
klaytn                    38.533505
unus sed leo              38.192922
shiba inu                 37.323294
fei usd                   35.201309
tether                    31.750932
...
bitcoin                   2.601950
gala                      2.470232
trust wallet token        2.461280
unus sed leo              2.422543
cronos                    2.072085
fei usd                   2.006800
tron                      1.846790
decentraland              1.805444
stellar                   1.621517
nem                       1.593857
ecash                     1.388415
bitcoin sv                1.306500
bitcoin gold              1.287142
